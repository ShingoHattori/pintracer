...
func sendICMPEchoRequest(destination string, TTL int, c *net.PacketConn) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("failed to parse reply message: %v", err)
	}
	...
	case ipv4.ICMPTypeEchoReply:
		...
		if !ok {
			return "", fmt.Errorf("invalid peer address type")
		}
	...
	case ipv4.ICMPTypeTimeExceeded:
		...
		return peerAddr.String(), nil
	default:
		return "", fmt.Errorf("unexpected message type: %v", rm.Type)
	}
}
...
func main() {
	...
	replyHost, err := sendICMPEchoRequest(dest, 1, &c)
	if err != nil {
		fmt.Println("Error:", err)
		return
	}
